      ...
      <div className="px-6 md:px-10 py-8 space-y-10 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div key={m.id} className="flex items-center gap-4 py-3 px-3 -mx-3 rounded-lg hover:bg-white/[0.04] transition-colors cursor-default">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <div className="h-[3px] flex-1 max-w-[100px] rounded-full bg-white/[0.04] overflow-hidden">
          ...
          {breaking.length > 0 ? (
            ...
            <section className="surface-section p-6 flex flex-col justify-center items-start gap-3">
              <div className="flex items-center gap-3">
                <Zap size={16} className="text-white/30" />
                <span className="text-sm font-semibold text-white/50">Modèles tendance</span>
              </div>
              <div className="space-y-2 w-full">
                {ranked.slice(0, 3).map(m => (
                  <Link key={m.id} href={`/models/${m.id}`} className="flex items-center justify-between py-1.5 px-2 -mx-2 rounded-lg hover:bg-white/[0.04] transition-colors">
                    <span className="text-[13px] text-white/70">{m.name}</span>
                    <span className="text-[12px] font-mono text-data">{m.scores.arena_elo}</span>
                  </Link>
                ))}
              </div>
              <Link href="/leaderboard" className="text-[12px] text-white/30 hover:text-white/60 mt-1 transition-colors">
